...
# Add place-specific question patterns
if "information_model" in intents:
    for topic in intents["information_model"].get("topics", []):
        name = topic["name"].lower()
        overview_questions = topic.get("question_patterns", [])
        for pattern in overview_questions:
            text = pattern["text"].replace("[PLACE]", name.title())
            word_list = nltk.word_tokenize(text)
            words.extend(word_list)
            documents.append((word_list, pattern["intent"]))
            if pattern["intent"] not in classes:
                classes.append(pattern["intent"])
...
def respond_to_query(intent, entities):
    if intent in ["ask_about_place", "general_info", "recommendation", "highlight_features"]:
        place = entities.get("place", "").lower()
        overview = get_place_overview(place)
        if overview:
            return (f"Here is some information about {place.title()}:\n"
                    f"{overview}")
        return f"Sorry, I don't have detailed information on {place.title()}."
